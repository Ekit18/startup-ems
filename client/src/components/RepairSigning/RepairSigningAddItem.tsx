...
export const RepairSigningAddItem: React.FC<RepairSigningAddItemProps> =
  observer(({ handleInputChange, isSigning }) => {
    ...
    const handleSelectChange = (
      ...
      handleInputChange(
        carOperations.find(
          (carOperation) => carOperation.id === Number(event.target.value),
        ),
      )
    }
            ...
            <Col md={12} className="d-flex justify-content-between">
              <Form>
                ...
                >
                  ...
                  {carOperations.map((carOperation) => (
                    <option key={carOperation.id} value={carOperation.id}>
                      {carOperation.name} {carOperation.price}
                    </option>
                  ))}
                </Form.Select>
              </Form>
