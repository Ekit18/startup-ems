...
export const RepairSigningShowItem: React.FC<RepairSigningShowItemProps> = observer(({ item, handleCarOperationDelete, isSigning }) => {
    return (
        <>
            <Container>
                <Row>
                    <Col md={12} className="d-flex justify-content-between">
                        <h2>{item.name}</h2>
                        <h2>{item.price}</h2>
                        <Button disabled={isSigning} onClick={() => handleCarOperationDelete(item.id)}>delete</Button>
                    </Col>
                </Row>
            </Container>
        </>
